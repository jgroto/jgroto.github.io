...
module Jekyll
	...
	class TipuePage 
			...
			def initialize(title,tags,url,text)
				@title  = title
				@tags =tags
				@url =url
				@text=text
	...
	class TipueGenerator < Generator
		...
		def generate(site)
			...
			pages=Array.new
			target = File.open('assets/src/tipuesearch/tipuesearch_content_unedited.js', 'w')
			target.truncate(target.size)
			target.puts('var tipuesearch = {"pages": [')
			
			# all_but_last, last = site.posts[0..-2], site.posts.last
			all_but_last, last = site.pages[0..-2], site.pages.last
			...
			all_but_last.each do |page|
				...
				tp_page = TipuePage.new(page.data['title'],page.data['tags'].to_s .concat(' ').concat(page.data['categories'].to_s),page.url,page.data['excerpt'].to_s)
				target.puts(tp_page.to_json + ',')
				
			end
			
			#Do the last
			tp_page = TipuePage.new(last.data['title'],last.data['tags'].to_s.concat(' ').concat(last.data['categories'].to_s),last.url,last.data['excerpt'].to_s)
			target.puts(tp_page.to_json)
